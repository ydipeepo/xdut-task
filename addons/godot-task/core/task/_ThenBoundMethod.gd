...
static func create(
	...
	if bind_args.is_empty():
		return _THEN_METHOD_CLASS.create(antecedent_task, method, name)
	...
	match method_argc - bind_args.size():
		0:
			if not _is_valid_bound_method_0(method, bind_args):
				_AUTOLOAD_CLASS.print_error(&"BAD_METHOD_ARGUMENT_SIGNATURE", method.get_method())
				return _CANCELED_CLASS.create(name)
		1:
			if not _is_valid_bound_method_1(method, bind_args):
				_AUTOLOAD_CLASS.print_error(&"BAD_METHOD_ARGUMENT_SIGNATURE", method.get_method())
				return _CANCELED_CLASS.create(name)
		2:
			if not _is_valid_bound_method_2(method, bind_args):
				_AUTOLOAD_CLASS.print_error(&"BAD_METHOD_ARGUMENT_SIGNATURE", method.get_method())
				return _CANCELED_CLASS.create(name)
		_:
...
static func _is_valid_bound_method_0(method: Callable, bind_args: Array) -> bool:
	if \
...
static func _is_valid_bound_method_1(method: Callable, bind_args: Array) -> bool:
	if \
		_AUTOLOAD_CLASS.has_current() and \
		_AUTOLOAD_CLASS.get_current().get_enable_strict_method_validation() and \
		not _AUTOLOAD_CLASS.is_lambda(method):
...
static func _is_valid_bound_method_2(method: Callable, bind_args: Array) -> bool:
	if \
		_AUTOLOAD_CLASS.has_current() and \
		_AUTOLOAD_CLASS.get_current().get_enable_strict_method_validation() and \
		not _AUTOLOAD_CLASS.is_lambda(method):
