...
func test_from() -> void:
	...
	if "配列 3 要素":
		...
		g = Scope.new(); t = Task.from_v([g, &"signal_1", 1])
		assert(t.is_pending)
		g.emit_signal_1()
		assert(t.is_completed)
		...
		g = Scope.new(); t = Task.from_v([g, &"signal_1", [Scope.ARG1]])
		assert(t.is_pending)
	...
	if "配列 2 要素":
		...
		g = Scope.new(); t = Task.from_v([g, &"from_immediate"])
		assert(t.is_completed)
		assert(await t.wait() == null)
		assert(t.is_completed)

		g = Scope.new(); t = Task.from_v([g, &"signal_0"])
		assert(t.is_pending)
		g.emit_signal_0()
		assert(t.is_completed)
	...
	if "配列 1 要素":
		...
		g = Scope.new(); t = Task.from_v([Task.completed()])
		assert(t.is_completed)
		assert(await t.wait() == null)
		assert(t.is_completed)
		
		g = Scope.new(); t = Task.from_v([Task.canceled()])
		assert(t.is_canceled)
		assert(await t.wait() == null)
		assert(t.is_canceled)
		
		g = Scope.new(); t = Task.from_v([Task.defer()])
		assert(t.is_pending)
		await wait_defer()
		assert(t.is_completed)
		assert(await t.wait() == null)
		...
		g = ScopeWithWait.new(); t = Task.from_v([g])
		assert(t.is_completed)
		assert(await t.wait() == null)
		assert(t.is_completed)

		g = ScopeWithCompleted.new(); t = Task.from_v([g])
		assert(t.is_pending)
		g.emit()
		assert(t.is_completed)
		assert(await wait_equal(t, []))
...
func test_all() -> void:
	...
	t = Task.all_v([], Cancel.canceled())
	assert(t.is_canceled)
	assert(not await wait_equal(t, []))
	assert(t.is_canceled)

	t = Task.all_v([], Cancel.deferred())
	assert(t.is_completed)
	assert(await wait_equal(t, []))
	assert(t.is_completed)
	...
	if "直値":
		...
		for d: int in 2:
			...
			p.push(Scope.ARG3)
			while p.next():
				t = Task.all_v(p.get_inputs())
				assert(t.is_completed)
				assert(await wait_equal(t, p.get_outputs()))
				assert(t.is_completed)
				
				t = Task.all_v(p.get_inputs(), Cancel.canceled())
				assert(t.is_canceled)
				assert(not await wait_equal(t, p.get_outputs()))
				assert(t.is_canceled)
				
				t = Task.all_v(p.get_inputs(), Cancel.deferred())
				assert(t.is_completed)
				assert(await wait_equal(t, p.get_outputs()))
				assert(t.is_completed)
...
func test_all_settled() -> void:
	...
	t = Task.all_settled_v([])
	assert(t.is_completed)
	assert(await wait_equal(t, []))
	assert(t.is_completed)

	t = Task.all_settled_v([], Cancel.canceled())
	assert(t.is_canceled)
	assert(not await wait_equal(t, []))
	assert(t.is_canceled)

	t = Task.all_settled_v([], Cancel.deferred())
	assert(t.is_completed)
	assert(await wait_equal(t, []))
	assert(t.is_completed)
	...
	if "直値":
		...
		for d: int in 2:
			...
			p.push(Scope.ARG3)
			while p.next():
				t = Task.all_settled_v(p.get_inputs())
				assert(t.is_completed)
				assert(await wait_equal_deep(t, p.get_inputs()))
				assert(t.is_completed)

				t = Task.all_settled_v(p.get_inputs(), Cancel.canceled())
				assert(t.is_canceled)
				assert(not await wait_equal_deep(t, p.get_inputs()))
				assert(t.is_canceled)
				
				t = Task.all_settled_v(p.get_inputs(), Cancel.deferred())
				assert(t.is_completed)
				assert(await wait_equal_deep(t, p.get_inputs()))
				assert(t.is_completed)
...
func test_any() -> void:
	...
	t = Task.any_v([], Cancel.canceled())
	assert(t.is_canceled)
	assert(await t.wait() == null)
	assert(t.is_canceled)

	t = Task.any_v([], Cancel.deferred())
	assert(t.is_canceled)
	assert(await t.wait() == null)
	assert(t.is_canceled)
	...
	if "直値":
		...
		for d: int in 2:
			...
			p.push(Scope.ARG3)
			while p.next():
				t = Task.any_v(p.get_inputs())
				assert(t.is_completed)
				assert(await t.wait() == p.get_output(0))
				assert(t.is_completed)
				
				t = Task.any_v(p.get_inputs(), Cancel.canceled())
				assert(t.is_canceled)
				assert(await t.wait() == null)
				assert(t.is_canceled)
				
				t = Task.any_v(p.get_inputs(), Cancel.deferred())
				assert(t.is_completed)
				assert(await t.wait() == p.get_output(0))
				assert(t.is_completed)
...
func test_race() -> void:
	...
	t = Task.race_v([], Cancel.canceled())
	assert(t.is_canceled)
	assert(await t.wait() == null)
	assert(t.is_canceled)

	t = Task.race_v([], Cancel.deferred())
	assert(t.is_pending)
	assert(await t.wait() == null)
	assert(t.is_canceled)
	...
	if "直値":
		...
		for d: int in 2:
			...
			while p.next():
				...
				t = Task.race_v(p.get_inputs(), Cancel.canceled())
				assert(t.is_canceled)
				u = await t.wait()
				assert(u == null)
				assert(t.is_canceled)
				
				t = Task.race_v(p.get_inputs(), Cancel.deferred())
				assert(t.is_completed)
				u = await t.wait()
				assert(u is Task)
				assert(u.is_completed)
...
func test_then() -> void:
	...
	if "配列 2 要素":
		...
		g = Scope.new(); t = Task.completed().then_v([g, &"from_immediate"])
		assert(t.is_completed)
		assert(await t.wait() == null)
		assert(t.is_completed)
	...
	if "配列 1 要素":
		...
		g = ScopeWithWait.new(); t = Task.completed().then_v([g])
		assert(t.is_completed)
		assert(await t.wait() == null)
		assert(t.is_completed)
	...
	if "他":
		...
		g = Scope.new(); t = Task.completed().then_v(Task.completed())
		assert(t.is_completed)
		assert(await t.wait() == null)
		assert(t.is_completed)
...
func test_any_index() -> void:
	...
	t = Task.any_index_v([], Cancel.canceled())
	assert(t.is_canceled)
	assert(await t.wait() == null)
	assert(t.is_canceled)

	t = Task.any_index_v([], Cancel.deferred())
	assert(t.is_canceled)
	assert(await t.wait() == null)
	assert(t.is_canceled)
	...
	if "直値":
		...
		for d: int in 2:
			...
			p.push(Scope.ARG3)
			while p.next():
				t = Task.any_index_v(p.get_inputs())
				assert(t.is_completed)
				assert(await t.wait() == 0)
				assert(t.is_completed)
				
				t = Task.any_index_v(p.get_inputs(), Cancel.canceled())
				assert(t.is_canceled)
				assert(await t.wait() == null)
				assert(t.is_canceled)
				
				t = Task.any_index_v(p.get_inputs(), Cancel.deferred())
				assert(t.is_completed)
				assert(await t.wait() == 0)
				assert(t.is_completed)
...
func test_all_count() -> void:
	...
	t = Task.all_count_v([], Cancel.canceled())
	assert(t.is_canceled)
	assert(await t.wait() == null)
	assert(t.is_canceled)

	t = Task.all_count_v([], Cancel.deferred())
	assert(t.is_completed)
	assert(await t.wait() == 0)
	assert(t.is_completed)
	...
	if "直値":
		...
		for d: int in 2:
			...
			p.push(Scope.ARG3)
			while p.next():
				t = Task.all_count_v(p.get_inputs())
				assert(t.is_completed)
				assert(await t.wait() == p.get_outputs().size())
				assert(t.is_completed)

				t = Task.all_count_v(p.get_inputs(), Cancel.canceled())
				assert(t.is_canceled)
				assert(await t.wait() == null)
				assert(t.is_canceled)
				
				t = Task.all_count_v(p.get_inputs(), Cancel.deferred())
				assert(t.is_completed)
				assert(await t.wait() == p.get_outputs().size())
				assert(t.is_completed)
